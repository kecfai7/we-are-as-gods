# -*- coding: utf-8 -*-
"""
Sync session7_EN.md scripts to sessionsData.js for Week 7
"""
...
session7_md_path = r"c:\We_are_as_Gods\session7_EN.md"

# 1. Parse all slide scripts from session7_EN.md
with open(session7_md_path, "r", encoding="utf-8") as f:
    md_text = f.read()
...
print(f"Parsed {len(slide_scripts)} slide scripts from {session7_md_path}")
...
# 3. Update week 7 slides
week7 = next((s for s in data if s.get("weekNumber") == 7), None)
if not week7:
    raise ValueError("Week 7 not found in sessionsData.js")

for slide in week7.get("slides", []):
...
print(f"Successfully synced {len(slide_scripts)} slide scripts for Week 7 into sessionsData.js!")
