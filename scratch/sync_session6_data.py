# -*- coding: utf-8 -*-
"""
Sync session6_EN.md scripts to sessionsData.js for Week 6
"""
...
session6_md_path = r"c:\We_are_as_Gods\session6_EN.md"

# 1. Parse all slide scripts from session6_EN.md
with open(session6_md_path, "r", encoding="utf-8") as f:
    md_text = f.read()
...
print(f"Parsed {len(slide_scripts)} slide scripts from {session6_md_path}")
...
# 3. Update week 6 slides
week6 = next((s for s in data if s.get("weekNumber") == 6), None)
if not week6:
    raise ValueError("Week 6 not found in sessionsData.js")

for slide in week6.get("slides", []):
...
print(f"Successfully synced {len(slide_scripts)} slide scripts for Week 6 into sessionsData.js!")
