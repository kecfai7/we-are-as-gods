# -*- coding: utf-8 -*-
"""
Sync session4_EN.md scripts to sessionsData.js for Week 4
"""
...
session4_md_path = r"c:\We_are_as_Gods\session4_EN.md"

# 1. Parse all slide scripts from session4_EN.md
with open(session4_md_path, "r", encoding="utf-8") as f:
    md_text = f.read()
...
print(f"Parsed {len(slide_scripts)} slide scripts from {session4_md_path}")
...
# 3. Update week 4 slides
week4 = next((s for s in data if s.get("weekNumber") == 4), None)
if not week4:
    raise ValueError("Week 4 not found in sessionsData.js")

for slide in week4.get("slides", []):
...
print(f"Successfully synced {len(slide_scripts)} slide scripts for Week 4 into sessionsData.js!")
